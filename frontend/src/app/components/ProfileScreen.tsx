...
export function ProfileScreen({ onLogout }: ProfileScreenProps) {
  const { user, updateUserLocally, refreshUser } = useUser();

  const [name, setName] = useState(user?.name || "User");
  const [email, setEmail] = useState(user?.email || "user@aura.ai");
  const [commStyle, setCommStyle] = useState(user?.communication_style || "balanced");
  const [interestsStr, setInterestsStr] = useState((user?.interests || []).join(", "));
  const [goalsStr, setGoalsStr] = useState((user?.goals || []).join(", "));
  ...
  useEffect(() => {
    if (user) {
      ...
      if (user.communication_style) setCommStyle(user.communication_style);
      if (user.interests) setInterestsStr(user.interests.join(", "));
      if (user.goals) setGoalsStr(user.goals.join(", "));
    }
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.allSettled([
        apiClient.patch("/api/v1/users/me", { name, communication_style: commStyle }),
        apiClient.put("/api/v1/users/me/interests", { interests }),
        apiClient.put("/api/v1/users/me/goals", { goals }),
      ]);
      await refreshUser();
      setSaved(true);
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleSave}
            className="clay-button flex items-center gap-2 px-6 py-2.5 text-xs font-bold text-[#7B59DC] cursor-pointer border-none outline-none"
